...
//Implements Converter with generic types Jwt and abstract Authentication Token
public class TwitterQueryUserJwtConverter implements Converter<Jwt, AbstractAuthenticationToken> {
//    Defined some constants that will refer to claims in JWT.
...
//    Implementation of custom JWT decoder to include new custom validators for claims like audience or nonce.
//    Moreover spring security by default includes authorities with SCOP_prefix for the scopes defined in JWT.
//    In order to add roles of a user and customize authorities to spring security authorities we need custom JWT converter/decoder is required.
//
    @Override
    public AbstractAuthenticationToken convert(Jwt jwt) {
        //Get Authorities from the received JWT token
        Collection<GrantedAuthority> authoritiesFromJwt = getAuthoritiesFromJwt(jwt);
        return Optional.ofNullable(
//              Get the username from the rcvd JWT token and get UserDetails object based on the username
                twitterQueryUserDetailsService.loadUserByUsername(jwt.getClaimAsString(USERNAME_CLAIM))).
//              Typecast UserDetails to TwitterQueryUser and set authorities received. Based on UsreDetails object create an
//        authentication token for the authenticate user.
                map(userDetails -> {
    ...
    private Collection<String> getRoles(Jwt jwt){
        ...
        if(roles instanceof Collection){
            return  ((Collection<String>) roles).stream()
                    .map(authority -> DEFAULT_ROLE_PREFIX+authority.toUpperCase()).collect(Collectors.toList());
        }
        return Collections.emptyList();
    }
    //Get authorities from Scopes
